...
register_page(
    __name__,
    name='Colonialism Sentiment Analysis',
    title='Mehrotra IBMS',
    path='/colonialism/dashboard'
)

# Fetch and preprocess the data
df = get_data()

# Filter the data for "Colonialism" and "Colonialist" keywords only once
df = df.loc[(df['Keyword'] == 'Colonialism') | (df['Keyword'] == 'Colonialist')]
...
keyword_options = [{'label': 'Colonialism', 'value': 'Colonialism'}, {'label': 'Colonialist', 'value': 'Colonialist'}]
...
layout = dbc.Container([
    dbc.Row([
        dbc.Col(html.A(html.Img(src='../assets/images/info.png', height='70px'), href='#', id='colonialism-info-icon'), width=1, className="text-right")
    ], style={'padding-bottom': '15px'}),
    dbc.Row([
        dbc.Col(html.H2("Colonialism & Colonialist", className="text-center my-4", style={'fontFamily': 'Roboto', 'color': 'white'})),
    ], justify="center"),
    dbc.Tooltip(
        [
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='colonialism-sentiment-over-time-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=6),
        dbc.Col([
            dcc.Graph(id='colonialism-high-low-indicator', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=3),
        dbc.Col([
            dcc.Graph(id='colonialism-overall-sentiment-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=3),
    ], className="mb-4"),
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='colonialism-emotion-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=6),
        dbc.Col([
            dcc.Graph(id='colonialism-publication-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=6),
    ], className="mb-4"),
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='colonialism-keyword-distribution-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=12)
    ], className="mb-4"),
...
@callback(
    Output('colonialism-sentiment-over-time-graph', 'figure'),
    Output('colonialism-high-low-indicator', 'figure'),
    Output('colonialism-overall-sentiment-graph', 'figure'),
    Output('colonialism-emotion-graph', 'figure'),
    Output('colonialism-publication-graph', 'figure'),
    Output('colonialism-keyword-distribution-graph', 'figure'),
    [Input('colonialism-keyword-radio', 'value'),
     Input('colonialism-emotions-radio', 'value'),
     Input('colonialism-year-slider', 'value')]
)
