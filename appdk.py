...
app.layout = html.Div([
    dbc.Container([
        dbc.Row([
            ...
            dbc.Col(html.A(html.Img(src=r'C:\\Users\\mamar\\Questrom Sentiment Project\\\\DASHBOARD\\assets\\infoicon.svg', height='30px'), href='#', id='info-icon'), width=1, className="text-right")
        ], justify="center"),
...
def update_graph(selected_source, selected_publisher, selected_keyword, selected_emotions, aggregation_value, selected_years):
    ...
    filtered_df_numeric = filtered_df[['Date'] + numeric_columns].set_index('Date')
    ...
    if filtered_df_numeric.empty:
        ...
        if aggregation_value == 'mean':
            ...
            yearly_sentiments = filtered_df_numeric.resample('YE').median().reset_index()
        elif aggregation_value == 'max':
            yearly_sentiments = filtered_df_numeric.resample('YE').max().reset_index()
        elif aggregation_value == 'min':
            yearly_sentiments = filtered_df_numeric.resample('YE').min().reset_index()
        elif aggregation_value == 'rolling_mean':
            yearly_sentiments = filtered_df_numeric.resample('YE').mean().rolling(window=5, min_periods=1).mean().reset_index()

        line_fig = px.line(yearly_sentiments, x='Date', y=numeric_columns,
    ...
    stacked_filtered_df = filtered_df[['Date', 'Positive', 'Negative']].set_index('Date')
    ...
    if stacked_filtered_df.empty:
        ...
        yearly_stacked_sentiments = stacked_filtered_df.resample('YE').mean().reset_index()
        ...
        stacked_area_fig.add_trace(go.Scatter(
            x=yearly_stacked_sentiments['Date'], 
    ...
    if aggregation_value == 'mean':
        animation_df = filtered_df.melt(id_vars=['Date', 'Year', 'Source Type', 'Publication Title', 'Keyword'], 
