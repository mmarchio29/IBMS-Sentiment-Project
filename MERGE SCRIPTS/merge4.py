...
final.drop_duplicates(subset=['ID', 'Keyword'], keep='first', inplace=True)
columns_to_drop = ['ID', 'Title_y', 'Publication', 'Date_y', 'Publication ID', 'ISBN', 'Authors']  
final.drop(columns = columns_to_drop, inplace=True)
final.rename(columns={'Date_x': 'Date', 'Title_x': 'Title'}, inplace = True)
