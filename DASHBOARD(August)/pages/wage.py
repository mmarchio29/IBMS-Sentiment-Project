...
from data_manager import get_data, get_wage
...
layout = dbc.Container([
    dbc.Row([
        dbc.Col(html.A(html.Img(src='../assets/images/info.png', height='70px'), href='#', id='wage-info-icon'), width=1, className="text-right")
    ]),
    dbc.Row([
        dbc.Col(html.H2("Economic Ideology Sentiment v. Median Weekly Real Earnings", className="text-center my-4", style={'fontFamily': 'Roboto', 'color': 'white'})),
    ], justify="center"),
    dbc.Tooltip(
        [
        ...
        target="wage-info-icon",
        placement="right"
    ),
    dbc.Row([
        dbc.Col(create_dropdown('wage-publication-type-dropdown', "Publication:", 
                                [{'label': 'All', 'value': 'All'}] + [{'label': st, 'value': st} for st in df['Publication'].unique() if st],
                                'All', multi=True), width=9, md=3),
        dbc.Col(create_dropdown('wage-keyword-dropdown', "Keyword:", 
                                [{'label': 'All', 'value': 'All'}] + [{'label': keyword, 'value': keyword} for keyword in df['Keyword'].unique() if keyword],
                                'All', multi=True), width=9, md=3),
        dbc.Col(create_dropdown('wage-emotions-dropdown', "Emotions:", 
        ...
        dbc.Col(create_dropdown('wage-aggregation-dropdown', "Aggregation Value:", 
    ...
    dbc.Row([
        dbc.Col([
            ...
            dcc.RangeSlider(
                id='wage-year-slider',
                min=int(wage_df['Year'].min()),
                max=int(wage_df['Year'].max()),
                value=[int(wage_df['Year'].min()), int(wage_df['Year'].max())],
                marks={**{str(year): str(year) for year in range(int(wage_df['Year'].min()), int(wage_df['Year'].max()) + 1) if year % 5 == 0},
                    str(wage_df['Year'].min()): str(wage_df['Year'].min()),
                    str(wage_df['Year'].max()): str(wage_df['Year'].max())},
                step=1,
    ...
    dbc.Row([
        dbc.Col([
            html.Div(id='wage-graph-container')
        ], width=12),
    ], className="mb-4"),
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='wage-trend-graph', style={'height': '400px'})
        ], width=12),
    ], className="mb-4"),
...
# Define callback
@callback(
    [Output('wage-graph-container', 'children'),
     Output('wage-trend-graph', 'figure')],
    [Input('wage-publication-type-dropdown', 'value'),
     Input('wage-keyword-dropdown', 'value'),
     Input('wage-emotions-dropdown', 'value'),
     Input('wage-aggregation-dropdown', 'value'),
     Input('wage-year-slider', 'value')]
)
def update_wage_graphs(selected_publication, selected_keyword, selected_emotions, aggregation_value, selected_years):
    ...
    # Index Graph using wage_df
    wage_filtered_df = wage_df[(wage_df['Year'] >= selected_years[0]) & (wage_df['Year'] <= selected_years[1])]

    # Ensure the dataframe is sorted by Year before plotting
    wage_filtered_df = wage_filtered_df.sort_values(by='Year')

    wage_fig = px.line(
        wage_filtered_df,
        x='Year',
        y='Dollars',
        labels={'Year': 'Year', 'Dollars': 'CPI Adjusted Dollars'},
        title='Median Weekly Real Earnings Over Time'
    )
    wage_fig.update_layout(**dark_theme)

    return sentiment_graphs, wage_fig