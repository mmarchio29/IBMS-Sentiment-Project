...
register_page(
    __name__,
    name='Imperialism Sentiment Analysis',
    title='Mehrotra IBMS',
    path='/imperialism/dashboard'
)

# Fetch and preprocess the data
df = get_data()

# Filter the data for "Imperialism" and "Imperialist" keywords only once
df = df.loc[(df['Keyword'] == 'Imperialism') | (df['Keyword'] == 'Imperialist')]
...
keyword_options = [{'label': 'Imperialism', 'value': 'Imperialism'}, {'label': 'Imperialist', 'value': 'Imperialist'}]
...
layout = dbc.Container([
    dbc.Row([
        dbc.Col(html.A(html.Img(src='../assets/images/info.png', height='70px'), href='#', id='imperialism-info-icon'), width=1, className="text-right")
    ], style={'padding-bottom': '15px'}),
    dbc.Row([
        dbc.Col(html.H2("Imperialism & Imperialist", className="text-center my-4", style={'fontFamily': 'Roboto', 'color': 'white'})),
    ], justify="center"),
    dbc.Tooltip(
        [
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='imperialism-sentiment-over-time-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=6),
        dbc.Col([
            dcc.Graph(id='imperialism-high-low-indicator', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=3),
        dbc.Col([
            dcc.Graph(id='imperialism-overall-sentiment-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=3),
    ], className="mb-4"),
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='imperialism-emotion-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=6),
        dbc.Col([
            dcc.Graph(id='imperialism-publication-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=6),
    ], className="mb-4"),
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='imperialism-keyword-distribution-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=12)
    ], className="mb-4"),
...
@callback(
    Output('imperialism-sentiment-over-time-graph', 'figure'),
    Output('imperialism-high-low-indicator', 'figure'),
    Output('imperialism-overall-sentiment-graph', 'figure'),
    Output('imperialism-emotion-graph', 'figure'),
    Output('imperialism-publication-graph', 'figure'),
    Output('imperialism-keyword-distribution-graph', 'figure'),
    [Input('imperialism-keyword-radio', 'value'),
     Input('imperialism-emotions-radio', 'value'),
     Input('imperialism-year-slider', 'value')]
)
