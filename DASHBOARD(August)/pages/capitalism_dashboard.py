...
register_page(
    __name__,
    name='Capitalism Sentiment Analysis',
    title='Mehrotra IBMS',
    path='/capitalism/dashboard'
)

# Fetch and preprocess the data
df = get_data()

# Filter the data for "Capitalism" and "Capitalist" keywords only once
df = df.loc[(df['Keyword'] == 'Capitalism') | (df['Keyword'] == 'Capitalist')]
...
keyword_options = [{'label': 'Capitalism', 'value': 'Capitalism'}, {'label': 'Capitalist', 'value': 'Capitalist'}]
...
layout = dbc.Container([
    dbc.Row([
        dbc.Col(html.A(html.Img(src='../assets/images/info.png', height='70px'), href='#', id='capitalism-info-icon'), width=1, className="text-right")
    ], style={'padding-bottom': '15px'}),
    dbc.Row([
        dbc.Col(html.H2("Capitalism & Capitalist", className="text-center my-4", style={'fontFamily': 'Roboto', 'color': 'white'})),
    ], justify="center"),
    dbc.Tooltip(
        [
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='capitalism-sentiment-over-time-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=6),
        dbc.Col([
            dcc.Graph(id='capitalism-high-low-indicator', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=3),
        dbc.Col([
            dcc.Graph(id='capitalism-overall-sentiment-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=3),
    ], className="mb-4"),
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='capitalism-emotion-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=6),
        dbc.Col([
            dcc.Graph(id='capitalism-publication-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=6),
    ], className="mb-4"),
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='capitalism-keyword-distribution-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=12)
    ], className="mb-4"),
...
@callback(
    Output('capitalism-sentiment-over-time-graph', 'figure'),
    Output('capitalism-high-low-indicator', 'figure'),
    Output('capitalism-overall-sentiment-graph', 'figure'),
    Output('capitalism-emotion-graph', 'figure'),
    Output('capitalism-publication-graph', 'figure'),
    Output('capitalism-keyword-distribution-graph', 'figure'),
    [Input('capitalism-keyword-radio', 'value'),
     Input('capitalism-emotions-radio', 'value'),
     Input('capitalism-year-slider', 'value')]
)
