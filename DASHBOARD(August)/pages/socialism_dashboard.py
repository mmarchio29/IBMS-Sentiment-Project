...
register_page(
    __name__,
    name='Socialism Sentiment Analysis',
    title='Mehrotra IBMS',
    path='/socialism/dashboard'
)

# Fetch and preprocess the data
df = get_data()

# Filter the data for "Socialism" and "Socialist" keywords only once
df = df.loc[(df['Keyword'] == 'Socialism') | (df['Keyword'] == 'Socialist')]
...
keyword_options = [{'label': 'Socialism', 'value': 'Socialism'}, {'label': 'Socialist', 'value': 'Socialist'}]
...
layout = dbc.Container([
    dbc.Row([
        dbc.Col(html.A(html.Img(src='../assets/images/info.png', height='70px'), href='#', id='socialism-info-icon'), width=1, className="text-right")
    ], style={'padding-bottom': '15px'}),
    dbc.Row([
        dbc.Col(html.H2("Socialism & Socialist", className="text-center my-4", style={'fontFamily': 'Roboto', 'color': 'white'})),
    ], justify="center"),
    dbc.Tooltip(
        [
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='socialism-sentiment-over-time-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=6),
        dbc.Col([
            dcc.Graph(id='socialism-high-low-indicator', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=3),
        dbc.Col([
            dcc.Graph(id='socialism-overall-sentiment-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=3),
    ], className="mb-4"),
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='socialism-emotion-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=6),
        dbc.Col([
            dcc.Graph(id='socialism-publication-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=6),
    ], className="mb-4"),
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='socialism-keyword-distribution-graph', style={'height': '400px', 'border': '1px solid #444'}),
        ], width=12)
    ], className="mb-4"),
...
@callback(
    Output('socialism-sentiment-over-time-graph', 'figure'),
    Output('socialism-high-low-indicator', 'figure'),
    Output('socialism-overall-sentiment-graph', 'figure'),
    Output('socialism-emotion-graph', 'figure'),
    Output('socialism-publication-graph', 'figure'),
    Output('socialism-keyword-distribution-graph', 'figure'),
    [Input('socialism-keyword-radio', 'value'),
     Input('socialism-emotions-radio', 'value'),
     Input('socialism-year-slider', 'value')]
)
